...
kable(sora_sig[, c("Symbol", "log2FoldChange", "padj")], 
      col.names = c("Symbol", "log2FC", "adj. p-value"), 
      caption = "Signifikante DEGs Sorafenib vs DMSO") %>%
  kable_styling() %>%
...
```{r Trametinib vs DMSO}
# Tabellarische Darstellung der signifikanten DEGs für Trametinib vs DMSO
kable(tram_sig[, c("Symbol", "log2FoldChange", "padj")], 
      col.names = c("Symbol", "log2FC", "adj. p-value"), 
      caption = "Signifikante DEGs Trametinib vs DMSO") %>%
  kable_styling() %>%
  scroll_box(width = "100%", height = "400px")
```
...
ggplot(gsea_down_plot, aes(x = -log10(adj.P.Val), y = Term)) +
  geom_point(aes(size = Count, color = adj.P.Val)) +
  scale_color_viridis_c(option = "plasma", direction = -1, name = "adj. p-value") +
  labs(
    title = "Dotplot: Top 10 Enriched Hallmark Gene Sets (Down-regulated)",
    x = "-log10(adj. p-value)",
    y = "Gene Set",
    size = "Gene Count"
  ) +
